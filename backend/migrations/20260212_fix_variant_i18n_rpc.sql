-- ============================================================================
-- FIX: Use -> operator for JSONB extraction to avoid string/casting issues
-- and prevent potential double-encoding or silent failures.
-- ============================================================================
...
BEGIN
    ...
    FOR v_variant IN SELECT * FROM jsonb_array_elements(p_variants) LOOP
        INSERT INTO product_variants (
            ...
            COALESCE(v_variant->'description_i18n', '{}'::jsonb), -- Fixed: Use -> for JSONB and COALESCE
            v_variant->>'hsn_code',
...
BEGIN
    ...
    SET
        ...
        title_i18n = p_product_data->>'title_i18n',
        description_i18n = p_product_data->>'description_i18n',
        benefits_i18n = p_product_data->>'benefits_i18n',
        tags_i18n = p_product_data->>'tags_i18n',
        updated_at = NOW()
    ...
    FOR v_variant IN SELECT * FROM jsonb_array_elements(p_variants) LOOP
        ...
        IF v_variant_id IS NOT NULL AND v_variant_id = ANY(v_existing_variant_ids) THEN
            ...
            SET
                ...
                description_i18n = COALESCE(v_variant->'description_i18n', '{}'::jsonb), -- Fixed
        ...
        ELSE
            ...
            INSERT INTO product_variants (
                ...
                (v_variant->>'is_default')::boolean,
                v_variant->>'description',
                COALESCE(v_variant->'description_i18n', '{}'::jsonb), -- Fixed
                v_variant->>'hsn_code',
                (v_variant->>'gst_rate')::decimal,
                (v_variant->>'tax_applicable')::boolean,
                (v_variant->>'price_includes_tax')::boolean
    ...
    v_result := jsonb_build_object(
        'product_id', v_product_id,
        'updated_variants', p_variants, -- Return the updated variants
