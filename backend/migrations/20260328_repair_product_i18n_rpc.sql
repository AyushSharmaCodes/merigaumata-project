-- Migration: Repair product i18n persistence in product-with-variants RPCs
-- Created: 2026-03-28
-- Description:
--   Re-adds localized product and variant fields that were dropped by a later
--   RPC replacement. Without this, admin saves can silently discard i18n JSONB
--   values for products created/updated through the variants flow.
...
BEGIN
    ...
    FOR i IN 1..array_length(p_variants, 1) LOOP
        IF COALESCE((p_variants[i]->>'is_default')::boolean, false) = true THEN
            v_has_default := true;
            EXIT;
        END IF;
    END LOOP;
    ...
    INSERT INTO products (
        ...
        COALESCE(ARRAY(SELECT jsonb_array_elements_text(p_product_data->'images')), '{}'::text[]),
        p_product_data->>'category',
        NULLIF(p_product_data->>'category_id', '')::uuid,
        COALESCE((p_product_data->>'inventory')::integer, 0),
        COALESCE(ARRAY(SELECT jsonb_array_elements_text(p_product_data->'tags')), '{}'::text[]),
        COALESCE(p_product_data->'tags_i18n', '{}'::jsonb),
        COALESCE(ARRAY(SELECT jsonb_array_elements_text(p_product_data->'benefits')), '{}'::text[]),
        COALESCE(p_product_data->'benefits_i18n', '{}'::jsonb),
        COALESCE((p_product_data->>'is_returnable')::boolean, (p_product_data->>'isReturnable')::boolean, true),
        COALESCE((p_product_data->>'return_days')::integer, (p_product_data->>'returnDays')::integer, 3),
        COALESCE((p_product_data->>'is_new')::boolean, (p_product_data->>'isNew')::boolean, false),
        COALESCE((p_product_data->>'variant_mode')::variant_mode_type, 'UNIT'::variant_mode_type),
        ...
        COALESCE((p_product_data->>'created_at')::timestamptz, (p_product_data->>'createdAt')::timestamptz, NOW())
    )
    ...
    FOREACH v_variant IN ARRAY p_variants LOOP
        INSERT INTO product_variants (
            ...
            COALESCE(v_variant->'size_label_i18n', jsonb_build_object('en', v_variant->>'size_label')),
...
BEGIN
    ...
    UPDATE products SET
        ...
        images = CASE
            WHEN p_product_data ? 'images' THEN ARRAY(SELECT jsonb_array_elements_text(p_product_data->'images'))
            ELSE images
        END,
        ...
        tags = CASE
            WHEN p_product_data ? 'tags' THEN ARRAY(SELECT jsonb_array_elements_text(p_product_data->'tags'))
            ELSE tags
        END,
        tags_i18n = COALESCE(p_product_data->'tags_i18n', tags_i18n),
        benefits = CASE
            WHEN p_product_data ? 'benefits' THEN ARRAY(SELECT jsonb_array_elements_text(p_product_data->'benefits'))
            ELSE benefits
        END,
        benefits_i18n = COALESCE(p_product_data->'benefits_i18n', benefits_i18n),
        is_returnable = COALESCE((p_product_data->>'is_returnable')::boolean, (p_product_data->>'isReturnable')::boolean, is_returnable),
        return_days = COALESCE((p_product_data->>'return_days')::integer, (p_product_data->>'returnDays')::integer, return_days),
        is_new = COALESCE((p_product_data->>'is_new')::boolean, (p_product_data->>'isNew')::boolean, is_new),
        variant_mode = COALESCE((p_product_data->>'variant_mode')::variant_mode_type, variant_mode),
