-- Migration: Add Variant Description and I18n Support
-- Created: 2026-02-12
-- Description: Adds description and description_i18n columns to product_variants and updates RPCs
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        COMMENT ON COLUMN product_variants.description IS 'Variant-specific description (defaults to English)';
    END IF;

    -- Add description_i18n if not exists
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'product_variants' AND column_name = 'description_i18n'
    ) THEN
        ALTER TABLE product_variants ADD COLUMN description_i18n JSONB DEFAULT '{}'::jsonb;
        COMMENT ON COLUMN product_variants.description_i18n IS 'Localized variant descriptions';
    END IF;
END $$;

-- ============================================================================
-- 2. UPDATE RPCs to handle new fields
-- ============================================================================
...
BEGIN
    ...
    -- Create the product (Standard Fields)
    ...
    FOREACH v_variant IN ARRAY p_variants LOOP
        INSERT INTO product_variants (
            ...
            COALESCE(v_variant->'description_i18n', '{}'::jsonb),             -- ADDED
        ...
        FOREACH v_variant IN ARRAY p_variants LOOP
            IF v_variant ? 'id' AND v_variant->>'id' != '' THEN
                ...
                UPDATE product_variants SET
                    ...
                    description_i18n = COALESCE(v_variant->'description_i18n', description_i18n),  -- ADDED
