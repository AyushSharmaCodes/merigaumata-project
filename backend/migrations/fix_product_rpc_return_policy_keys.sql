-- Migration: Fix Product RPC Return Policy Keys (camelCase -> snake_case)
-- Created: 2026-02-11
-- Description: Updates RPC functions to match the snake_case keys sent by the service layer.

-- ============================================================================
-- 1. CREATE PRODUCT WITH VARIANTS (Fixed keys)
-- ============================================================================
...
BEGIN
    ...
    INSERT INTO products (
        ...
        -- FIX: Use is_returnable (snake_case) or fallback to camelCase just in case, default true
        COALESCE((p_product_data->>'is_returnable')::boolean, (p_product_data->>'isReturnable')::boolean, true),
        -- FIX: Use return_days (snake_case) or fallback to camelCase just in case, default 3
        COALESCE((p_product_data->>'return_days')::integer, (p_product_data->>'returnDays')::integer, 3),
        -- FIX: Add is_new support
...
-- ============================================================================
-- 2. UPDATE PRODUCT WITH VARIANTS (Fixed keys)
-- ============================================================================
...
BEGIN
    ...
    UPDATE products SET
        ...
        -- FIX: Check for is_returnable (snake) then isReturnable (camel)
        is_returnable = COALESCE((p_product_data->>'is_returnable')::boolean, (p_product_data->>'isReturnable')::boolean, is_returnable),
        -- FIX: Check for return_days (snake) then returnDays (camel)
