-- Migration: Fix Dashboard Revenue Calculation
-- Created: 2026-04-01
-- Description: Updates the get_admin_dashboard_stats_v3 function to correctly count orders with 'paid' and 'captured' status as revenue.
    ...
    IF p_scope = 'all' OR p_scope = 'summary' THEN
        ...
        core_counts AS (
            SELECT
                ...
                (SELECT COALESCE(SUM(total_amount), 0) FROM public.orders WHERE payment_status IN ('success', 'paid', 'captured')) as total_earnings,
        ...
        sparkline_metrics AS (
            SELECT
                ...
                (SELECT COALESCE(SUM(total_amount), 0) FROM public.orders o WHERE o.payment_status IN ('success', 'paid', 'captured') AND 
                    CASE WHEN p_summary_timeframe = 'yearly' THEN date_trunc('month', o.created_at) = date_trunc('month', sd.day)
    ...
    IF p_scope = 'all' OR p_scope = 'charts' THEN
        ...
        revenue_trend_data AS (
            SELECT
                ...
                (SELECT COALESCE(SUM(d.amount), 0) FROM public.donations d WHERE d.payment_status IN ('success', 'paid', 'captured') AND 
            ...
            LEFT JOIN public.orders o ON 
                ...
                AND o.payment_status IN ('success', 'paid', 'captured')
            GROUP BY gs, name
            ORDER BY gs
        ),
        ...
        category_stats AS (
            ...
            LEFT JOIN (
                ...
                WHERE o.payment_status IN ('success', 'paid', 'captured')
