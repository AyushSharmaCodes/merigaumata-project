-- Consolidated Dashboard Analytics RPC v7 (Summary Timeframe)
-- This function fetches dashboard statistics with modular scoping and dynamic summary timeframe filtering.
...
BEGIN
    ...
    IF p_scope = 'all' OR p_scope = 'summary' THEN
        ...
        core_counts AS (
            SELECT
                ...
                (SELECT COALESCE(SUM(amount), 0) FROM public.donations WHERE payment_status = 'success') as total_donations,
                (SELECT COALESCE(SUM(total_amount), 0) FROM public.orders WHERE payment_status = 'success') as total_earnings,
                ...
                (SELECT COALESCE(SUM(amount), 0) FROM public.donations WHERE payment_status = 'success' AND created_at >= v_summary_start_date) as new_donations_amount,
        ...
        sparkline_metrics AS (
            SELECT
                ...
                (SELECT COALESCE(SUM(amount), 0) FROM public.donations d WHERE d.payment_status = 'success' AND 
                ...
                (SELECT COALESCE(SUM(total_amount), 0) FROM public.orders o WHERE o.payment_status = 'success' AND 
                    CASE WHEN p_summary_timeframe = 'yearly' THEN date_trunc('month', o.created_at) = date_trunc('month', sd.day)
    ...
    IF p_scope = 'all' OR p_scope = 'charts' THEN
        v_start_date := CASE 
            WHEN p_revenue_timeframe = 'weekly' THEN NOW() - INTERVAL '7 days'
            WHEN p_revenue_timeframe = 'monthly' THEN NOW() - INTERVAL '30 days'
        ...
        revenue_trend_data AS (
            SELECT
                ...
                (SELECT COALESCE(SUM(d.amount), 0) FROM public.donations d WHERE d.payment_status = 'success' AND 
            ...
            FROM (
                SELECT generate_series(
                    date_trunc(CASE WHEN p_revenue_timeframe = 'yearly' THEN 'year' ELSE 'day' END, v_start_date),
            ...
            LEFT JOIN public.orders o ON 
                CASE WHEN p_revenue_timeframe = 'yearly' THEN date_trunc('month', o.created_at) = date_trunc('month', gs)
                     ELSE o.created_at::date = gs::date END
