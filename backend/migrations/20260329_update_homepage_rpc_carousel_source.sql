-- Update get_public_homepage_content to fetch carousel slides from the gallery folder 
-- marked as is_home_carousel instead of the old carousel_slides table.
...
BEGIN
    ...
    -- Fetch carousel slides from the gallery folder marked as is_home_carousel
    IF to_regclass('public.gallery_folders') IS NOT NULL AND to_regclass('public.gallery_items') IS NOT NULL THEN
        SELECT COALESCE(jsonb_agg(slide_row ORDER BY slide_row.order_index ASC), '[]'::jsonb)
        ...
        FROM (
            SELECT 
                ...
                gi.title,
                gi.title_i18n,
                gi.description AS subtitle,
                gi.description_i18n AS subtitle_i18n,
                gi.order_index,
                TRUE AS is_active,
                gi.created_at,
                gi.updated_at
            FROM public.gallery_items gi
            JOIN public.gallery_folders gf ON gf.id = gi.folder_id
...
GRANT EXECUTE ON FUNCTION public.get_public_homepage_content(TIMESTAMPTZ) TO anon;
GRANT EXECUTE ON FUNCTION public.get_public_homepage_content(TIMESTAMPTZ) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_public_homepage_content(TIMESTAMPTZ) TO service_role;
