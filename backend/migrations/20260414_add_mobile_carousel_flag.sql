-- Step 1: Add is_mobile_carousel column to gallery_folders
ALTER TABLE gallery_folders 
ADD COLUMN IF NOT EXISTS is_mobile_carousel BOOLEAN DEFAULT false;

-- Step 2: Create a unique index for the mobile carousel folder
-- This ensures only one folder can be marked as the mobile carousel
CREATE UNIQUE INDEX IF NOT EXISTS idx_gallery_folders_mobile_carousel 
ON gallery_folders (is_mobile_carousel) 
WHERE is_mobile_carousel = true;

-- Step 3: Update common RPC to return mobile carousel slides
    ...
    v_homepage := (
        SELECT jsonb_build_object(
            ...
            'mobileCarouselSlides', (
                ...
                FROM gallery_items gi
                JOIN gallery_folders gf ON gf.id = gi.folder_id
                WHERE gf.is_mobile_carousel = true 
                AND gf.is_active = true
                ORDER BY gi.order_index ASC
            ),
            'products', (
