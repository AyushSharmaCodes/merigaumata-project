...
import type { OrderStatusHistory, OrderViewState } from "@/shared/types";
...
export const OrderTimelineSection = memo(({
    ...
    const adminActions = viewState?.actions?.admin;
    const availableTransitions = adminActions?.available_status_transitions || [];
    const activeReturnRequestId = adminActions?.active_return_request_id || viewState?.lifecycle?.active_return_request_id || null;
    const activeReturnRequest = activeReturnRequestId
        ? returnRequests.find((request: any) => request.id === activeReturnRequestId) || null
        : null;
            ...
            <CardContent className="p-0">
                ...
                <div className="p-8 bg-slate-50/50 border-t border-slate-100 flex flex-col md:flex-row items-center justify-between gap-4">
                    <div className="flex flex-col gap-1">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-bold text-slate-700 uppercase tracking-tighter">
                                {(() => {
                                    if (availableTransitions.includes('confirmed')) return "Confirm Customer Order";
                                    if (availableTransitions.includes('processing')) return "Start Warehouse Processing";
                                    if (availableTransitions.includes('packed')) return "Pack Order Items";
                                    if (availableTransitions.includes('shipped')) return "Generate Shipping Labels";
                                    if (availableTransitions.includes('out_for_delivery') && currentStatus === 'shipped') return "Track Out For Delivery";
                                    if (availableTransitions.includes('delivered')) return "Confirm Delivery Receipt";
                                    if (availableTransitions.includes('delivery_reattempt_scheduled')) return "Choose Reattempt or Start RTO";
                                    if (availableTransitions.includes('out_for_delivery') && currentStatus === 'delivery_reattempt_scheduled') return "Dispatch Reattempt Delivery";
                                    if (availableTransitions.includes('returned_to_origin')) return "Confirm Return to Origin";
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        {availableTransitions.includes('packed') && (
                        ...
                        {availableTransitions.includes('out_for_delivery') && currentStatus === 'shipped' && (
                        ...
                        {availableTransitions.includes('delivery_unsuccessful') && (
                        ...
                        {availableTransitions.includes('delivery_reattempt_scheduled') && (
                        ...
                        {availableTransitions.includes('returned_to_origin') && (
                            <Button 
                        ...
                        {(() => {
                            if (activeReturnRequest && adminActions?.can_approve_active_return && adminActions?.can_reject_active_return) {
                                return (
                                    <div className="flex items-center gap-2">
                                        <Button onClick={() => onReturnAction(activeReturnRequest.id, 'approve')} disabled={isUpdating} className="bg-green-600 hover:bg-green-700 text-white font-bold text-xs h-10 px-5 rounded-xl shadow-lg shadow-emerald-200/50">
                                            Approve Return
                                        </Button>
                                        <Button variant="ghost" onClick={() => onReturnAction(activeReturnRequest.id, 'reject')} disabled={isUpdating} className="text-red-600 hover:bg-red-50 font-bold text-xs h-10 px-5 rounded-xl">
                                            Reject
                                        </Button>
                                    </div>
                                );
                            ...
                            if (activeReturnRequest && adminActions?.can_mark_active_return_picked_up) {
                                return (
                                    <Button onClick={() => onReturnAction(activeReturnRequest.id, 'picked_up')} disabled={isUpdating} className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs h-10 px-6 rounded-xl shadow-lg shadow-indigo-200/50">
                                        Confirm Picked Up
                                    </Button>
                                );
                            }
