import { logger, logAPICall, logAPIRequest, logFrontendAuthEvent, logPageAction } from "@/core/observability/logger";
import { getErrorMessage, isNetworkError } from "@/core/utils/errorUtils";
import axios, { AxiosError, InternalAxiosRequestConfig } from 'axios';
import { ApiErrorResponse, User } from "@/shared/types";
import { getGuestId } from '@/shared/lib/guest-id';
import { CONFIG } from "@/app/config";
import i18n from "@/app/i18n/config";
import { hasAcceptedCookieConsent, requestCookieConsentForCriticalAction, requiresCookieConsentForRequest, shouldAuditCookieConsentCoverage, CookieConsentRequiredError, getNormalizedConsentRequestPath, COOKIE_CONSENT_DECIDED_EVENT } from "@/shared/lib/cookie-consent";
import { clearAuthSession, clearAuthUserSnapshot, getAuthSession } from "@/domains/auth";
...
async function performRefreshRequest(
    ...
    while (true) {
        try {
            ...
            const res = await api.post<RefreshResponse>('/auth/refresh', body, { silent } as any);
    ...
    async (error: AxiosError) => {
        ...
        if (error.response?.status === 401 && originalRequest && !originalRequest._retry) {
            ...
            try {
                ...
                // api.request(originalRequest) will trigger request interceptors again
