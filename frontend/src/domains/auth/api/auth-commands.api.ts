...
import type { User, ApiErrorResponse } from "@/shared/types";
import axios from "axios";
import { UserDTO } from "@/domains/user/model/user.dto";
import { clearAuthSession, getAuthSession, setAuthSession } from "../services/auth-session.service";
...
export const exchangeGoogleCode = async (code: string, state: string): Promise<{ user: User; tokens?: { access_token: string; refresh_token: string } }> => {
  ...
  // Persist session marker — works whether tokens are in the body or only in cookies.
  setAuthSession(response.data?.tokens ?? {});
...
export const verifyLoginOtp = async (email: string, otp: string): Promise<User> => {
  ...
  // In HttpOnly cookie mode the backend doesn't include tokens in the JSON body.
  // Always set the session marker so bootstrap state and proactive refresh detect
  // an active session on subsequent page loads.
