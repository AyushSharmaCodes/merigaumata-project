...
import { OrderStatusHistory, ReturnRequest } from "@/types";
    ...
    const timelineItems = React.useMemo(() => {
        ...
        relevantHistory.forEach(h => {
            ...
            if (targetReturnId) {
                ...
                const isGlobal = h.status === 'delivered' || h.status === 'cancelled';
...
function TimelineEntry({ entry, isLatest }: { entry: OrderStatusHistory; isLatest: boolean }) {
    ...
    const isAmber = ['return_requested', 'pickup_scheduled', 'pickup_attempted'].includes(s) || s.includes('requested');
    const isRed = ['return_rejected', 'return_cancelled', 'qc_failed', 'pickup_failed'].includes(s);
