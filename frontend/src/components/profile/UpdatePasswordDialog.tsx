...
import { changePassword, sendChangePasswordOTP } from "@/lib/services/auth.service";
import {
    InputOTP,
    InputOTPGroup,
    InputOTPSeparator,
            ...
            if (details) {
                details.forEach((d) => {
                    ...
                    if (field && (field === 'currentPassword' || field === 'newPassword' || field === 'confirmPassword' || field === 'otp')) {
                        // We can't easily translate dynamic backend errors here unless we have a mapping, 
                        // but generally validation errors from Zod will catch most issues.
                        // If backend returns a message, we might want to display it or map it.
                        // For now, using the message as is or relying on t() if it returns a key.
